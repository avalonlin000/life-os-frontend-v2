...
> 交付同步默认不 @；@ 机制已废除，只有钧钧明确要求叫人接手时才使用结构化 @。
...
交付同步、进度汇报、普通结果通知默认不 @ 结衣/小雪，也不 @ 钧钧。只有钧钧明确要求“叫某人/让某人接手/必须唤醒某人”时，才使用结构化 @。

结构化 @ 仅作为明确接手指令的例外格式；具体 open_id 从当前消息 `bridge_context.mentions` 或 Hermes 当前通讯录取，不在项目文档硬编码。不要凭名字猜 open_id。

### 2. 交接闭环

群聊推进任务时，默认由小白主责闭环。只有下一步确实需要别人接手，且钧钧明确允许或要求唤醒对方时，才在末尾结构化 @ 下一位负责人；否则只写“给小雪/给结衣”的摘要分区，不唤醒。

### 3. 不要形成机器人互相 @ 死循环

不要小白、结衣、小雪互相无意义 @ 来回确认。交付同步不等于接手指令。

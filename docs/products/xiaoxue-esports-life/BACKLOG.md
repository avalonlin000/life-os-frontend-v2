...
| 9 | TK 检索迁移到 MemPalace | 已验证 | 714 份 Wiki TK 进入 `xiaoxue-tk`；8770 新入口已成为唯一运行入口，8880 工作台真实搜索回读通过；8768 服务、索引、日志和专属环境已删除，旧展示链接已移除 |
...
| ID | 任务 | 验收 | 状态 | 说明 |
|---|---|---|---|---|
| XX-P2-1 | 纯阵容输入边界 | 蓝红方十英雄 + 比赛版本；默认等经济、正常发育、同等操作水平 | Done | `lol-lineup-analysis` v8.0、产品事实源 |
| XX-P2-2 | 纯阵容输出合同 | 完整可见过程、版本契合、24场景、四阶段、固定32分30秒、比赛画像和观赛信号 | Done | `lol-lineup-analysis` v8.1、ACCEPTANCE |

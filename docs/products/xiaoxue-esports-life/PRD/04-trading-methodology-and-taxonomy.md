...
- 交易 TK 仍归属队伍 TK，不新增独立交易库；队内和日报按比赛优先显示。
...
- 工作台：主导航只保留“队伍资料 / 当前赛事 / TK资料库”。盘口记录作为隐藏辅助；今日日报、BP/阵容、小雪聊天、预案生成、三层对照和阶段复盘全部移出。
...
每日唯一默认动作是打开日报。工作台、对话和盘口记录都按需使用。
...
这套词表只保留真实有内容的对象；入口、字段、分析方式和旧别名不另起资料类别。

| 对象 | 正式叫法 | 说明 |
|---|---|---|
| 全局版本资料 | 版本理解 | `shared_version_analysis` 的全局版本结论 |
| 队伍资料总入口 | 队伍资料 | 下面只放画像、三维、队伍 TK |
| 队伍长期文字资料 | 画像 | Wiki / 队伍画像 Skill 的正式内容 |
| 队伍三维资料 | 三维 | 优势局、劣势局、胜负手和三维内战术笔记 |
| 队伍知识资料 | 队伍 TK | 队伍相关 TK 的统一归属 |
| 队伍交易指导 | 交易 TK | 归在队伍 TK 内，队内优先单列；日报按比赛优先展示 |
| TK 总入口 | TK资料库 | 保存和检索 TK |
| 单条知识记录 | TK | 不再叫“知识条目” |
| 关系展示 | 知识图 | 由 TK 生成的关系展示，不是第二套知识库 |
| 市场手写记录 | 盘口记录 | 对应 `market_notes` |
| 每日成品 | 日报 | 赛前交易判断日报归在日报下面 |
| 结构化实力资料 | TS 表 | 字段为实力均值、波动、保守实力下界 |
| 选手上场信息 | 首发 | 来自 `rosters` |
| BP 后英雄组合 | 阵容 | 蓝红双方十个英雄 |
| 时间安排 / 对局 / 结果 | 赛程 / 比赛 / 赛果 | 三者继续分工 |

“版本环境”“版本类型”“版本知识”“版本认知”只作为输入兼容，回复统一使用“版本理解”。“横向基本面”只保留为后台分析方式，不作为用户栏目；“赛前内容卡”没有真实存档，不属于产品词表。

队伍原 `team_3d_data.version_understanding` 字段已完成迁移：筛选后的 30 条有效内容归入对应队伍 TK，1 条测试内容跳过；该字段保留为空仅作内部兼容，不再展示或写入。
...
2. 交易前只允许使用已经存在的资料：赛程、TS 表、三维、画像、队伍 TK、交易 TK、舆论材料包。
...
6. 沉淀必须回到实体和方法：交易 TK 回队伍 TK，阵容和 BP 回单场分析，复盘结论回三维、画像、TK 或日报。
...
| F10 | 队伍三维查看与编辑 | `/api/team-3d/{team}`, `PUT /api/team-3d/{team}` | 查看/维护三维和三维内战术笔记 | team、三维字段 | 三维数据、更新时间 | 钧钧 / 系统 | 是，维度是 LOL 队伍语义 | 部分可复用，维度需按项目重建 |
| F11 | 队伍 TK 优先展示 | `/api/team-trading-notes/{team}`, `/api/tk/search` | 在队伍 TK 内优先展示交易 TK；不建立独立交易库 | team、TK 文件 | 交易 TK、普通队伍 TK | 钧钧 / 模型 | 是 | 是，队伍知识优先层可复用 |
...
| F20 | 交易 TK | `/api/team-trading-notes`, `/api/team-trading-notes/from-text` | 把“小雪记到 HLE：...”写回队伍 TK，结构化为 `type=trading_note` | 自然语言备注或结构化备注 | 队伍 TK 文件、active/inactive 交易 TK | 钧钧 / 模型 / 系统 | 是，队伍别名当前 LOL | 是，实体备注模式可复用 |
...
| F11 队伍 TK 优先展示 | 可扩展项目 | 长期 / 交易前 | 队伍 / 知识 / 市场 | 展示 / 组装 | 钧钧 / 模型 | 沉淀 / 解释 |
...
| F20 交易 TK | 可扩展项目 | 长期 / 交易前 / 交易后 | 队伍 / 市场 / 知识 | 记录 / 判断 / 沉淀 | 钧钧 / 模型 / 系统 | 沉淀 |
...
当前结论：队伍资料是第一项目的长期依据入口；横向基本面只作后台分析方式，不是用户栏目。
...
- TS 表：实力均值、波动、保守实力下界、risk_gap、资料置信度。
- 队伍长期依据：画像、三维、队伍 TK、概念图。
- 市场对照：盘口、赔率、市场是否简化强弱或忽略波动。
- 交易 TK：active `type=trading_note`，跟随对应队伍 TK。
...
- 盘口记录：钧钧自己手写判断，保存到 `market_notes`。
...
| VersionContext | 版本理解 | `shared_version_analysis`；旧队伍版本字段已迁移至队伍 TK |
...
4. 工作台主导航固定为“队伍资料 / 当前赛事 / TK资料库”，盘口记录只作隐藏辅助。

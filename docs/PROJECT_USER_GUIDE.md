...
交付同步默认不 @、不发飞书。项目上下文同步主机制是 delivery 文件：`/home/ubuntu/life-os-frontend-v2/.hermes/deliveries/latest.md`。只有钧钧明确要求“叫某人/让某人接手/必须唤醒某人”时，才使用结构化 @；具体 open_id 从当前消息 mentions 或 Hermes 当前通讯录取，不在项目文档硬编码。
...
- 如果涉及跨机器人协作，默认先写明交接对象、原因和上下文；只有钧钧明确要求叫人接手/唤醒时才结构化 @。
